...
def translate_ebook_content(content: str, lang_input: str = 'fr', lang_output: str = 'en') -> str:
    ...
    input_text = text[lang_input]
    
    output_text = text[lang_output]
...
def process_file(file_path, translation_status, csv_file, lang_input: str, lang_output: str):
    ...
    # save before and after for may sure doesn't error
    ...
    for element in track(text_elements, description=f'[cyan]translation {file}'):
        ...
        if isinstance(element, str) and element.parent.name not in ['script', 'style', 'dc:identifier', 'identifier']:
            if not isinstance(element, Comment):
                ...
                if text:
                    while True:
                        try:
                            chunks = split_text_into_chunks(text, max_tokens=5000)
    ...
    # keep /!\ very important for initialyze content and correctly use after -> re.search(r'<\?xml[^>]+\?>', content) or file.write(content)
    content = str(soup)
    
    
    # regex expresion for finds balise arround both dot
    pattern = re.compile(r'([.,;?:])\s*(<[^>]+>)\s*(<[^>]+>)\s*([.,;?:])')

    # func for remove dot with last regex expresion
    def remove_dot(match):
        # remove dot
        tag = match.group()[1:]

        return f'{tag}'
    
    # new content without first dot
    content = pattern.sub(remove_dot, content)

    
    # replace translation bugs
    content = content.replace('&gt;', '>').replace('&lt;', '<')
